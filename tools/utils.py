...
def collate_intermediate_frame(frame_list):
    '''
        frame_list: list of frames
    '''
    new_frame_list = []
    for example in frame_list:
        example_collated = collate_kitti(example)
        example_gpu = example_to_device(example_collated, torch.cuda.current_device(), non_blocking=False)
        new_frame_list.append(example_gpu)
    return new_frame_list

def find_nearest(X, Y):
    return torch.argmin(                                      #  (X - Y) ** 2 
